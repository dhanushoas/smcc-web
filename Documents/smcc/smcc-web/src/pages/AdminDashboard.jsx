...
const AdminDashboard = () => {
    ...
    const calculateWinner = (matchData) => {
        ...
        if (inn1.runs > inn2.runs) {
            return `${inn1.team.toUpperCase()} WON BY ${inn1.runs - inn2.runs} RUNS`;
        } else if (inn2.runs > inn1.runs) {
            const wicketsRemaining = 10 - inn2.wickets;
            return `${inn2.team.toUpperCase()} WON BY ${wicketsRemaining} WICKETS`;
        }
        return "MATCH DRAWN";
    };
    ...
    const handleUpdate = async (type, value, params = {}) => {
        setIsUpdating(true);
        let updatedMatch = { ...selectedMatch, innings: [...scorecardData], score: { ...selectedMatch.score } };
        ...
        if (type === 'manual') {
            ...
            if (type === 'init') {
                ...
                if (type === 'runs') {
                    ...
                    updatedMatch.score.thisOver.push(value.toString());
                    ...
                    let extraLabel = value.toUpperCase();
                    if (value === 'w') {
                    ...
                    updatedMatch.score.thisOver.push(extraLabel);
                    ...
                    // This handles simple OUT button if you want, but we use WicketModal
                ...
                if (ballCounts && type !== 'wicket_with_replacement' && type !== 'retired_with_replacement' && type !== 'new_bowler') {
                    ...
                    let bOvers = currentBowling.bowling[bIdx].overs || 0;
                    ...
                    if (ballCount >= 6) {
                        ...
                        const temp = localStriker; localStriker = localNonStriker; localNonStriker = temp;
                        updatedMatch.score.thisOver = []; // Clear history after over
    ...
    const calculateMOM = (match) => {
        ...
        winnerInn.batting.forEach(p => {
            candidates[p.player] = { runs: p.runs, wickets: 0 };
        });
        // Bowling points (25 points per wicket)
                    ...
                    {isCreating ? (
                        ...
                        <Card className="shadow-lg border-0 overflow-hidden">
                            ...
                            <Card.Body className="p-4">
                                <div className="text-center mb-4 bg-light rounded-4 p-4 border shadow-sm position-relative">
                                    {selectedMatch.status === 'live' && (
                                        <div className="position-absolute top-0 start-0 m-3">
                                            <Badge bg="danger" className="animate-pulse">● LIVE</Badge>
                                        </div>
                                    )}
                                    <div className="display-3 fw-black text-primary mb-2" style={{ letterSpacing: '-2px' }}>
                                        {selectedMatch.score.runs}/{selectedMatch.score.wickets}
                                    </div>
                                    <div className="fs-5 fw-bold text-muted mb-3">
                                        {selectedMatch.score.overs} / {selectedMatch.totalOvers} OVERS
                                    </div>
                                    <div className="d-flex justify-content-center gap-2 flex-wrap">
                                        <Badge bg="white" text="dark" className="border px-3 py-2 shadow-sm rounded-pill fw-bold">CRR: {crr}</Badge>
                                        {rrr && <Badge bg="info" text="white" className="px-3 py-2 shadow-sm rounded-pill fw-bold">RRR: {rrr}</Badge>}
                                        {selectedMatch.score.target && <Badge bg="warning" text="dark" className="px-3 py-2 shadow-sm rounded-pill fw-bold">TARGET: {selectedMatch.score.target}</Badge>}
                                    </div>

                                    {selectedMatch.score.thisOver && selectedMatch.score.thisOver.length > 0 && (
                                        <div className="mt-4 pt-3 border-top">
                                            <div className="small fw-bold text-muted text-uppercase mb-2">Recent Balls</div>
                                            <div className="d-flex justify-content-center gap-2 overflow-auto pb-2">
                                                {selectedMatch.score.thisOver.map((ball, idx) => (
                                                    <div key={idx} className={`d-flex align-items-center justify-content-center rounded-circle fw-bold shadow-sm ${ball.includes('W') ? 'bg-danger text-white' : (ball.includes('wd') || ball.includes('nb') ? 'bg-warning text-dark' : 'bg-white border text-dark')}`} style={{ minWidth: '32px', height: '32px', fontSize: '11px' }}>
                                                        {ball}
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {selectedMatch.status === 'live' && (
                                    <>
                                        <Row className="g-3 mb-4">
                                            <Col xs={12} md={6}>
                                                <Card className="border-0 shadow-sm overflow-hidden h-100" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
                                                    <div className="bg-primary bg-opacity-10 px-3 py-2 d-flex justify-content-between align-items-center">
                                                        <span className="small fw-black text-primary text-uppercase letter-spacing-1">Batting</span>
                                                        <Button variant="link" size="sm" className="p-0 text-decoration-none fw-bold x-small" onClick={() => handleUpdate('swap_strike')}>⇄ SWAP</Button>
                                                    </div>
                                                    <Card.Body className="p-3">
                                                        <div className="d-flex flex-column gap-2">
                                                            {selectedMatch.currentBatsmen?.map((b, i) => (
                                                                <div key={i} className={`d-flex justify-content-between align-items-center p-2 rounded-3 ${b.onStrike ? 'bg-white shadow-sm border-start border-primary border-4' : 'opacity-75'}`}>
                                                                    <div className="fw-bold text-truncate me-2" style={{ maxWidth: '120px' }}>
                                                                        {b.onStrike ? '🏏 ' : ''}{b.name || '...'}
                                                                    </div>
                                                                    <div className="fw-black text-primary">
                                                                        {b.runs}<small className="text-muted fw-bold">({b.balls})</small>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                            ...
                                            <Col xs={12} md={6}>
                                                <Card className="border-0 shadow-sm overflow-hidden h-100" style={{ background: 'linear-gradient(135deg, #f0fdfa 0%, #ccfbf1 100%)' }}>
                                                    <div className="bg-success bg-opacity-10 px-3 py-2 d-flex justify-content-between align-items-center">
                                                        <span className="small fw-black text-success text-uppercase letter-spacing-1">Bowling</span>
                                                        <Button variant="link" size="sm" className="p-0 text-decoration-none fw-bold x-small text-success" onClick={() => setShowBowlerModal(true)}>CHANGE</Button>
                                                    </div>
                                                    <Card.Body className="p-3 d-flex align-items-center justify-content-center text-center">
                                                        <div>
                                                            <div className="fw-black fs-5 text-success mb-1">⚾ {bowler || '...'}</div>
                                                            <div className="small fw-bold text-muted">
                                                                {scorecardData.find(inn => inn.team !== selectedMatch.score.battingTeam)?.bowling.find(p => p.player === bowler)
                                                                    ? (b => `${b.overs} ov | ${b.runs} r | ${b.wickets} w`)(scorecardData.find(inn => inn.team !== selectedMatch.score.battingTeam).bowling.find(p => p.player === bowler))
                                                                    : 'Starting Over'}
                                                            </div>
                                                        </div>
                                                    </Card.Body>
                                        ...
                                        <div className="scoring-grid mb-4">
                                            <div className="small fw-black text-muted text-uppercase mb-3 letter-spacing-1">Quick Scores</div>
                                            <div className="d-flex flex-wrap gap-2 mb-3">
                                                {[0, 1, 2, 3].map(r => (
                                                    <Button key={r} variant="outline-primary" className="flex-fill py-3 fw-black rounded-3 shadow-sm border-2" onClick={() => handleUpdate('runs', r)}>{r}</Button>
                                                ))}
                                            </div>
                                            <div className="d-flex flex-wrap gap-2 mb-3">
                                                {[4, 6].map(r => (
                                                    <Button key={r} variant="primary" className="flex-fill py-3 fw-black rounded-3 shadow-sm bg-gradient border-0" onClick={() => handleUpdate('runs', r)}>{r} RUNS</Button>
                                                ))}
                                                <Button variant="danger" className="flex-fill py-3 fw-black rounded-3 shadow-sm bg-gradient border-0" onClick={() => { setWicketDetails({ type: 'caught', fielder: '' }); setShowWicketModal(true); }}>WICKET</Button>
                                            </div>
                                            <div className="d-flex flex-wrap gap-2">
                                                <Button variant="warning" className="flex-fill py-2 fw-bold rounded-pill text-white shadow-sm border-0" onClick={() => handleUpdate('extra', 'w')}>WIDE</Button>
                                                <Button variant="warning" className="flex-fill py-2 fw-bold rounded-pill text-white shadow-sm border-0" onClick={() => handleUpdate('extra', 'nb')}>NO BALL</Button>
                                                <Button variant="secondary" className="flex-fill py-2 fw-bold rounded-pill shadow-sm border-0" onClick={() => handleUpdate('extra', 'b')}>BYE</Button>
                                                <Button variant="secondary" className="flex-fill py-2 fw-bold rounded-pill shadow-sm border-0" onClick={() => handleUpdate('extra', 'lb')}>LEG BYE</Button>
                                            </div>
                                        </div>

                                        <div className="d-flex gap-2 mb-4">
                                            <Button variant="outline-info" className="flex-fill fw-bold rounded-pill" onClick={() => setShowRunOutModal(true)}>RUN OUT</Button>
                                            <Button variant="outline-danger" className="flex-fill fw-bold rounded-pill" onClick={() => { setBatsmanModalType('retired'); setShowBatsmanModal(true); }}>RETIRE BATS.</Button>
                                            <Button variant="dark" className="px-4 fw-bold rounded-pill" onClick={() => setShowSquadModal(true)}>SQUADS</Button>
                                        </div>
                                    </>
                                )}
                                ...
                                {selectedMatch.status === 'upcoming' && (
                                    <div className="text-center py-5">
                                        ...
                                        <h3 className="fw-black text-muted mb-4 uppercase letter-spacing-1">Match Not Started</h3>
                                        <div className="d-flex gap-3 justify-content-center">
                                            {!selectedMatch.toss?.winner ? (
                                                <Button variant="warning" size="lg" className="px-5 fw-black rounded-pill shadow" onClick={() => setShowTossModal(true)}>🪙 CONDUCT TOSS</Button>
                                            ) : (
                                                <Button variant="success" size="lg" className="px-5 fw-black rounded-pill shadow" onClick={() => setShowStartModal(true)}>🚀 START MATCH</Button>
                                            )}
                                            <Button variant="outline-dark" size="lg" className="px-4 fw-black rounded-pill" onClick={() => setShowSquadModal(true)}>👥 SQUADS</Button>
                                        </div>
                                    </div>
                                )}
                                ...
                                {selectedMatch.status === 'completed' && (
                                    <div className="text-center py-4">
                                        <div className="alert alert-success border-0 shadow py-4 px-5 rounded-4 d-inline-block">
                                            <div className="fs-1 mb-2">🏅</div>
                                            <h3 className="fw-black mb-1">{calculateWinner(selectedMatch)?.toUpperCase()}</h3>
                                            {selectedMatch.manOfTheMatch && (
                                                <div className="mt-2 fw-bold text-success fs-5">
                                                    🏅 MAN OF THE MATCH: {selectedMatch.manOfTheMatch.toUpperCase()}
                                                </div>
                                            )}
                                        </div>
                                        <div className="d-flex gap-3 justify-content-center mt-4 pt-3 border-top">
                                            <Button variant="outline-primary" size="lg" className="px-4 fw-bold rounded-pill shadow-sm" onClick={handleDownloadPDF}>📥 DOWNLOAD PDF</Button>
                                            <Button variant="outline-danger" size="lg" className="px-4 fw-bold rounded-pill shadow-sm" onClick={(e) => handleDelete(e, selectedMatch._id || selectedMatch.id)}>DELETE MATCH</Button>
                                        </div>
                                    </div>
                                )}

                                {selectedMatch.toss?.winner && selectedMatch.status !== 'completed' && (
                                    <Alert variant="warning" className="text-center fw-bold border-0 shadow-sm rounded-4 mt-2 mb-4">
                                        🪙 Toss won by {selectedMatch.toss.winner} and elected to {selectedMatch.toss.decision} first.
                                    </Alert>
                                )}

                                <details className="mt-5">
                                    <summary className="btn btn-sm btn-link text-decoration-none fw-bold p-0 text-muted opacity-50">🔧 Correction Panel (Manual Overrides)</summary>
                                    <Card className="mt-2 border-0 bg-light p-4 shadow-sm rounded-4">
                                        <Row className="g-4">
                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Total Runs</Form.Label>
                                                <Form.Control className="border-0 shadow-sm rounded-3" type="number" value={selectedMatch.score.runs} onChange={e => handleUpdate('manual', { ...selectedMatch, score: { ...selectedMatch.score, runs: parseInt(e.target.value) || 0 } })} />
                                            </Col>
                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Wickets</Form.Label>
                                                <Form.Control className="border-0 shadow-sm rounded-3" type="number" value={selectedMatch.score.wickets} onChange={e => handleUpdate('manual', { ...selectedMatch, score: { ...selectedMatch.score, wickets: parseInt(e.target.value) || 0 } })} />
                                            </Col>
                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Overs</Form.Label>
                                                <Form.Control className="border-0 shadow-sm rounded-3" type="number" step="0.1" value={selectedMatch.score.overs} onChange={e => handleUpdate('manual', { ...selectedMatch, score: { ...selectedMatch.score, overs: parseFloat(e.target.value) || 0 } })} />
                                            </Col>

                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Striker</Form.Label>
                                                <Form.Select className="border-0 shadow-sm rounded-3" value={striker} onChange={e => { setStriker(e.target.value); handleUpdate('manual', { ...selectedMatch, currentBatsmen: selectedMatch.currentBatsmen.map((b, i) => i === 0 ? { ...b, name: e.target.value } : b) }); }}>
                                                    <option value="">Select</option>
                                                    {(selectedMatch.score.battingTeam === selectedMatch.teamA ? squadA : squadB).map(p => <option key={p} value={p}>{p}</option>)}
                                                </Form.Select>
                                            </Col>
                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Non-Striker</Form.Label>
                                                <Form.Select className="border-0 shadow-sm rounded-3" value={nonStriker} onChange={e => { setNonStriker(e.target.value); handleUpdate('manual', { ...selectedMatch, currentBatsmen: selectedMatch.currentBatsmen.map((b, i) => i === 1 ? { ...b, name: e.target.value } : b) }); }}>
                                                    <option value="">Select</option>
                                                    {(selectedMatch.score.battingTeam === selectedMatch.teamA ? squadA : squadB).map(p => <option key={p} value={p}>{p}</option>)}
                                                </Form.Select>
                                            </Col>
                                            <Col md={4}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Current Bowler</Form.Label>
                                                <Form.Select className="border-0 shadow-sm rounded-3" value={bowler} onChange={e => { setBowler(e.target.value); handleUpdate('manual', { ...selectedMatch, currentBowler: e.target.value }); }}>
                                                    <option value="">Select</option>
                                            ...
                                            <Col md={6}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Batting Team</Form.Label>
                                                <Form.Select className="border-0 shadow-sm rounded-3" value={selectedMatch.score.battingTeam} onChange={e => {
                                            ...
                                            <Col md={6}>
                                                <Form.Label className="x-small fw-black text-muted text-uppercase">Match Status</Form.Label>
                                                <Form.Select className="border-0 shadow-sm rounded-3" value={selectedMatch.status} onChange={e => handleUpdate('manual', { ...selectedMatch, status: e.target.value })}>
